...
- 页面路径统一使用 `/:lang/...` 结构（例如 `/en/aptos/contract-interaction`、`/zh/aptos/bcs-tools`），复制链接即可保留语言和工具上下文。
...
5. 导航栏中的 Aptos 钱包按钮统一管理连接/断开操作，任意 Aptos 工具都会保持同步。
6. 选择签名方式：
   - **Aptos 钱包**：集成官方 Wallet Adapter React，可直接连接 Petra 等浏览器钱包并发起交易。
   - **私钥签名**：输入发送者私钥（Hex 格式）后提交。
7. 点击“提交交易”即可完成签名与发送，返回交易哈希与 Explorer 链接。
