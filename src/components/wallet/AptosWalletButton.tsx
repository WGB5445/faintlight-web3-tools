...
async function fetchAptBalance(restUrl: string, accountAddress: string): Promise<string> {
...
export default function AptosWalletButton({
  ...
  const [balance, setBalance] = useState<BalanceState>({ loading: false, value: null, error: null });
  const [actionError, setActionError] = useState<string | null>(null);
  const [copied, setCopied] = useState(false);

  const buttonLabel = connected && address ? truncateAddress(address, 8, 6) : t('aptos.walletButton.connect');
  ...
  useEffect(() => {
    ...
    fetchAptBalance(restUrl, address)
      ...
      .catch((error) => {
        if (!cancelled) {
          setBalance({ loading: false, value: null, error: error instanceof Error ? error.message : String(error) });
        }
      });
    return () => {
      cancelled = true;
  ...
  return (
    <Fragment>
      ...
      >
        <span className="inline-block h-2 w-2 rounded-full" style={{ backgroundColor: connected ? '#38bdf8' : '#64748b' }} />
        <span>{buttonLabel}</span>
      </button>

      {renderModal ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-sm" onClick={closeModal}>
          <div
          ...
          >
            ...
            <div className="space-y-6">
              <header className="space-y-1">
                <h2 className="text-lg font-semibold">{t('aptos.walletButton.modalTitle')}</h2>
                <p className="text-sm text-slate-400">
                  {connected && walletName
                    ? t('aptos.signing.connectedSummary', { wallet: walletName })
              ...
              <section className="space-y-4 rounded-xl border border-slate-800 bg-slate-900/40 p-4">
                <div className="space-y-3 text-sm text-slate-300">
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500">{t('aptos.walletButton.walletName')}</span>
                    <span className="rounded-md bg-slate-900/70 px-3 py-1 text-xs text-slate-200">{walletName ?? '—'}</span>
                  </div>
                  <div>
                    <span className="text-slate-500">{t('aptos.walletButton.address')}</span>
                    <div className="mt-1 flex items-center justify-between gap-3 rounded-md border border-slate-800 bg-slate-900/70 px-3 py-2 text-xs">
                      <span className="truncate text-slate-200">{address || '—'}</span>
                      <button
                        type="button"
                      ...
                      >
                        {copied ? t('aptos.walletButton.copied') : t('aptos.walletButton.copyAddress')}
                      </button>
                    </div>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500">{t('aptos.walletButton.network')}</span>
                    <span className="rounded-md bg-slate-900/70 px-3 py-1 text-xs text-slate-200">{networkLabel}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500">{t('aptos.walletButton.balance')}</span>
                    <span className="rounded-md bg-slate-900/70 px-3 py-1 text-xs text-slate-200">{balanceText}</span>
                  </div>
                </div>
              ...
              <section className="space-y-3 rounded-xl border border-slate-800 bg-slate-900/40 p-4">
                <h3 className="text-sm font-semibold text-slate-200">{t('aptos.walletButton.availableWallets')}</h3>
                {availableWallets.length > 0 ? (
                    ...
                    {availableWallets.map((item) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-xs text-slate-500">{t('aptos.walletButton.statusLabel', { status: item.readyState })}</span>
                          <span className={`mt-1 text-xs font-medium ${
                            isCurrent ? 'text-sky-200' : 'text-sky-300'
                          }`}>
                            {isCurrent ? t('aptos.walletButton.currentWallet') : t('aptos.walletButton.connectAction')}
                          </span>
                        </button>
                      );
                    })}
                  ...
                  <p className="text-xs text-slate-500">{t('aptos.walletButton.noWallets')}</p>
                )}
              </section>
