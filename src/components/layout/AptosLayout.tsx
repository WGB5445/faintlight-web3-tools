...
import {
  ...
  useAptosSettings,
} from "../../context/AptosSettingsContext";
...
function isSelectableWallet(
  wallet: ReturnType<typeof useWallet>["wallets"][number],
) {
...
function AptosHeaderAccessory() {
  ...
  const { t } = useLanguage();
  const { wallets, connect, disconnect, connected, account, wallet, network } =
    useWallet();
  ...
  const availableWallets = useMemo(
    () =>
      wallets
        ...
        .map((item) => ({
          name: item.name,
          readyState: item.readyState ?? "NotDetected",
        })),
    [wallets],
  );
  ...
  const networkLabel =
    (network?.name as string) ?? t("aptos.signing.unknownNetwork");
