...
export default function AppShell() {
  ...
  const [headerAccessory, setHeaderAccessory] = useState<ReactNode | null>(
    null,
  );
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-8 px-6 pb-12 pt-10">
              ...
              <div className="flex w-full min-h-[48px] flex-wrap items-center gap-3 rounded-xl border border-slate-800 bg-slate-900/50 px-4 py-2 text-xs text-slate-400 md:justify-between">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-slate-500">
                    {t("layout.status.chain")}：
                  </span>
                  <span className="text-sky-300">
                    {t(activeContext.group.titleKey)}
                  </span>
                  <span className="px-2 text-slate-600">|</span>
                  <span className="text-slate-500">
                    {t("layout.status.tool")}：
                  </span>
                  <span className="text-slate-200">
                    {t(activeContext.item.labelKey)}
                  </span>
                </div>
                {headerAccessory ? (
                  <div className="ml-auto flex items-center text-left">
                    {headerAccessory}
                  </div>
                ) : null}
              </div>
        ...
        <div className="flex flex-col gap-6 lg:flex-row">
          ...
          <main className="min-h-[60vh] flex-1 rounded-2xl border border-slate-800 bg-slate-900/40 p-8 shadow-xl shadow-sky-950/20">
            <Outlet
              context={{ setHeaderAccessory: handleSetHeaderAccessory }}
            />
          </main>
        </div>
