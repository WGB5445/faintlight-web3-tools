...
type BasePrimitive = 'bool' | 'u8' | 'u16' | 'u32' | 'u64' | 'u128' | 'u256' | 'address' | 'string' | 'vector<u8>';
...
const BASE_TYPE_OPTIONS: Array<{ value: BasePrimitive; labelKey: string; hintKey: string }> = [
...
function buildTypeNode(base: BasePrimitive, wrappers: WrapperType[]): TypeNode {
  return wrappers.reduce<TypeNode>(
    (acc, wrapper) => (wrapper === 'vector' ? { kind: 'vector', inner: acc } : { kind: 'option', inner: acc }),
    buildBaseNode(base)
  );
}
...
function isNumericNode(node: TypeNode): boolean {
  return node.kind === 'primitive' && ['u8', 'u16', 'u32', 'u64', 'u128', 'u256'].includes(node.name);
}

function isVectorU8Node(node: TypeNode): boolean {
  return node.kind === 'vector' && node.inner.kind === 'primitive' && node.inner.name === 'u8';
}
...
export default function BcsToolPage() {
  ...
  const typeNode = useMemo(() => buildTypeNode(baseType, wrappers), [baseType, wrappers]);
  ...
  const buildArgumentFromValue = (node: TypeNode, value: unknown): BcsSerializable => {
    ...
    if (node.kind === 'vector') {
      ...
      if (!Array.isArray(value)) {
        throw new Error(t('bcs.errors.vectorRequiresArray', { type: describeType(node) }));
      }
      const items = value.map((item) => buildArgumentFromValue(node.inner, item)) as BcsSerializable[];
  ...
  const decodeValue = (node: TypeNode, deserializer: Deserializer): unknown => {
    if (node.kind === 'primitive') {
      switch (node.name) {
        ...
        default:
          throw new Error(t('bcs.errors.unsupportedType', { type: describeType(node) }));
      }
    }
  ...
  const handleRun = () => {
    ...
    try {
      ...
      if (typeNode.kind === 'primitive') {
        switch (typeNode.name) {
          ...
          case 'string': {
            ...
            setResult(text);
            setAuxiliary(t('bcs.results.rawBytes', { bytes: Buffer.from(text, 'utf8').toString('hex') }));
            return;
          }
          case 'address':
      ...
      if (isVectorU8Node(typeNode)) {
        ...
        setAuxiliary(t('bcs.results.vectorText', { text: Buffer.from(byteArray).toString('utf8') }));
        return;
      }
      ...
      if (typeNode.kind === 'option') {
        if (decoded === null) {
          ...
          setResult(t('bcs.results.optionSome', { value: formatJsonValue(decoded) }));
        }
        return;
      }
  ...
  const renderEncodeInput = () => {
    if (typeNode.kind === 'primitive' && typeNode.name === 'bool') {
      return (
        <label className="flex items-center gap-3 text-sm text-slate-200">
          ...
          />
          <span className="text-xs text-slate-500">{t('bcs.encode.boolHint')}</span>
        </label>
      );
    }
    ...
    if (typeNode.kind === 'primitive' && (typeNode.name === 'address' || isNumericNode(typeNode))) {
      return (
        <input
      ...
      <section className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <span className="text-sm font-medium text-slate-200">{t('bcs.typeSelect.wrapperLabel')}</span>
            ...
            {wrappers.length === 0 ? (
              <p className="text-xs text-slate-500">{t('bcs.typeSelect.wrapperEmpty')}</p>
            ) : (
              <div className="flex flex-wrap gap-2">
                {wrappers.map((wrapper, index) => (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      setWrappers((prev) => prev.filter((_, idx) => idx !== index));
                      resetOutputs();
                      resetInputsForTypeChange();
                    }}
            ...
            <p className="text-xs text-slate-500">{t('bcs.typeSelect.currentType', { type: readableType })}</p>
          </div>
        ...
        {result ? (
          <div className="space-y-3 rounded-xl border border-slate-800 bg-slate-900/60 p-5 text-sm text-slate-200">
            <div>
              ...
              <pre className="mt-1 whitespace-pre-wrap break-words font-mono text-xs text-sky-300">{result}</pre>
            </div>
            {auxiliary ? (
              <div>
                <p className="text-xs uppercase tracking-wide text-slate-500">{t('bcs.results.extraTitle')}</p>
                <pre className="mt-1 whitespace-pre-wrap break-words text-xs text-slate-300">{auxiliary}</pre>
              </div>
            ) : null}
          </div>
        ) : null}
