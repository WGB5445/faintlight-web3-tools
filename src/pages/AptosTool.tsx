...
import {
  ...
  parseTypeTagLite,
  substituteGenerics,
  type TypeTag as CustomTypeTag,
  encodeArgsPrimitivesTypes,
} from "../lib/aptos";
...
function extractTxnHash(value: unknown): string {
  ...
  if (typeof value === "object") {
    const hash =
      (value as Record<string, unknown>).hash ??
...
export default function AptosToolPage() {
  ...
  const [submissionMode, setSubmissionMode] =
    useState<SubmissionMode>("wallet");
  const [moduleId, setModuleId] = useState("");
  const [moduleLoading, setModuleLoading] = useState(false);
  const [moduleError, setModuleError] = useState<string | null>(null);
  const [moduleBytecode, setModuleBytecode] =
    useState<MoveModuleBytecode | null>(null);
  ...
  const normalizedWalletNetwork = walletNetwork?.name
    ? walletNetwork.name.toLowerCase()
    : null;
  const networkMismatch =
    submissionMode === "wallet" &&
    normalizedWalletNetwork &&
    networkId !== "custom" &&
    normalizedWalletNetwork !== networkId;
  ...
  const selectedFn = useMemo(
    () => entryFunctions.find((fn) => fn.name === selectedFunction),
    [entryFunctions, selectedFunction],
  );
  ...
  const parameterTypes = useMemo(
    () => callableParams.map((param) => parseTypeTagLite(param)),
    [callableParams],
  );
  ...
  const convertRawValue = useCallback(
    (
      index: number,
      functionName: string,
      functionAbiOrModuleAbi: FunctionABI,
      ...
      if (!tag) {
        throw new Error(
          t("aptos.errors.missingTypeInfo", { index: index + 1 }),
        );
      }
  ...
  const loadModule = useCallback(async () => {
    ...
    try {
      ...
      const firstEntry = moduleBytecode?.abi?.exposed_functions.find(
        (fn) => fn.is_entry,
      );
  ...
  const handleSubmit = async () => {
    ...
    if (trimmedTypeArgs.length !== selectedFn.generic_type_params.length) {
      setSubmission({
        status: "error",
        message: t("aptos.submission.typeArgMismatch"),
      });
      return;
    }
    if (
      trimmedTypeArgs.length > 0 &&
      trimmedTypeArgs.some((value) => value.length === 0)
    ) {
      setSubmission({
        status: "error",
        message: t("aptos.submission.typeArgMissing"),
      });
      return;
    }
    ...
    if (submissionMode === "wallet") {
      if (!connected || !account) {
        setSubmission({
          status: "error",
          message: t("aptos.submission.walletRequired"),
        });
        return;
      }
      ...
      try {
        ...
        if (!hash) {
          setSubmission({
            status: "error",
            message: t("aptos.submission.walletHashMissing"),
          });
          return;
        }
        ...
        setSubmission({
          ...
          hash,
          explorerUrl,
          message: t("aptos.submission.success"),
        });
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
    ...
    if (!privateKey.trim()) {
      setSubmission({
        status: "error",
        message: t("aptos.submission.privateKeyMissing"),
      });
      return;
    }
  ...
  const disableSubmit =
    isSubmitting || (submissionMode === "wallet" && !connected);

  const walletNetworkLabel =
    (walletNetwork?.name as string) ?? t("aptos.signing.unknownNetwork");
        ...
        <section className="space-y-6">
          <div className="space-y-3">
            <h2 className="text-xl font-semibold text-slate-100">
              {t("aptos.functions.title")}
            </h2>
            {entryFunctions.length === 0 ? (
              <p className="text-sm text-slate-400">
                {t("aptos.functions.empty")}
              </p>
            ) : (
              <select
                ...
                {entryFunctions.map((fn) => (
                  <option key={fn.name} value={fn.name}>
                    {t("aptos.functions.optionLabel", {
                      name: fn.name,
                      count: fn.params.filter(
                        (param) => !isSignerParameter(param),
                      ).length,
                    })}
                  </option>
                ))}
          ...
          {selectedFn && (
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                <h3 className="text-lg font-semibold text-slate-200">
                  {t("aptos.arguments.title")}
                </h3>
                {callableParams.length === 0 ? (
                  <p className="text-sm text-slate-400">
                    {t("aptos.arguments.none")}
                  </p>
                ) : (
                  <div className="space-y-6">
                    {callableParams.map((param, index) => {
                      ...
                      return (
                        <div
                          key={`${param}-${index}`}
                          className="rounded-xl border border-slate-800 bg-slate-900/60 p-4 shadow-inner"
                        >
                          <div className="flex flex-col gap-2 text-sm text-slate-300">
                            <span className="font-semibold text-slate-100">
                              {t("aptos.arguments.label", { index: index + 1 })}
                            </span>
                            <span className="text-xs uppercase tracking-wide text-slate-500">
                              {t("aptos.arguments.type", { type: tag.kind })}
                            </span>
                          </div>
                            ...
                            <div className="mt-4">
                              <textarea
                                ...
                                placeholder={
                                  ...
                                  tag.name === "String"
                                    ? t(
                                        "aptos.arguments.placeholders.rawString",
                                      )
                                    : t(
                                        "aptos.arguments.placeholders.rawGeneric",
                                      )
                                }
                              ...
                              tag.elem.kind === "u8" ? (
                                <p className="mt-1 text-xs text-slate-500">
                                  {t("aptos.arguments.hints.rawVector")}
                                </p>
                              ) : null}
                            </div>
                          )}
                          ...
                          {state?.mode === "hex" && (
                            <div className="mt-4">
                              <textarea
                                className="min-h-[90px] w-full rounded-lg border border-slate-700 bg-slate-950/70 px-3 py-2 text-sm text-slate-100"
                                placeholder={t(
                                  "aptos.arguments.placeholders.hex",
                                )}
                          ...
                          {state?.mode === "bcs" && (
                            <div className="mt-4">
                              <textarea
                                className="min-h-[90px] w-full rounded-lg border border-slate-700 bg-slate-950/70 px-3 py-2 text-sm text-slate-100"
                                placeholder={t(
                                  "aptos.arguments.placeholders.bcs",
                                )}
                  ...
                  {submissionMode === "wallet" ? (
                    <div className="space-y-3 text-sm text-slate-200">
                      {connected && activeWallet ? (
                        ...
                        <p className="text-xs text-slate-400">
                          {t("aptos.signing.reminderConnectAbove")}
                        </p>
                      )}
                      {networkMismatch ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  {submission.status === "success" && submission.hash ? (
                    <div className="space-y-2 rounded-lg border border-slate-800 bg-slate-900/60 p-4 text-sm text-slate-200">
                      {submission.message ? (
                        <p className="text-slate-300">{submission.message}</p>
                      ) : null}
                      <p>
                        {t("aptos.submission.hashLabel")}：
