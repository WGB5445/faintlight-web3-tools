...
export default function AptosAbiViewerPage() {
  ...
  const [activeStructTabs, setActiveStructTabs] = useState<Record<string, StructTabType>>({});
  ...
  const generateTsExample = (fn: any, moduleAbi: any, type: 'entry' | 'view') => {
  ...
  const renderFunction = (fn: any, type: 'entry' | 'view') => {
    ...
    return (
      <div key={fnId} className="rounded-lg border border-slate-700 bg-slate-900/60 overflow-hidden">
        {/* Header */}
        <div className="p-4 border-b border-slate-700">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-lg font-semibold text-slate-100 font-mono">{fn.name}</h3>
          </div>
        ...
        <div className="p-4">
          {activeTab === 'details' && (
            <div className="space-y-4">
              {fn.generic_type_params && fn.generic_type_params.length > 0 && (
                <div>
                  <h4 className="text-sm font-medium text-slate-300 mb-2">Generic Type Parameters</h4>
                  <div className="space-y-1">
                    {fn.generic_type_params.map((param: any, index: number) => (
                      <div key={index} className="text-sm text-slate-400 font-mono bg-slate-800/50 px-3 py-2 rounded">
                        <span className="text-slate-300">T{index}</span>
                        {param.constraints && param.constraints.length > 0 ? (
                          <span className="text-slate-500">: {param.constraints.join(', ')}</span>
                        ) : (
                          <span className="text-slate-500">: No constraints</span>
                        )}
                      </div>
                    ))}
                  </div>
              ...
              {fn.return && fn.return.length > 0 && (
                <div>
                  <h4 className="text-sm font-medium text-slate-300 mb-2">Return Types</h4>
                  <div className="space-y-1">
          ...
          {activeTab === 'moveCode' && (
            <div className="relative">
              ...
              >
                {copiedId === copyId ? t('aptosAbi.code.copied') : t('aptosAbi.code.copy')}
              </button>
  ...
  const renderStruct = (struct: any) => {
    ...
    return (
      <div key={structId} className="rounded-lg border border-slate-700 bg-slate-900/60 overflow-hidden">
        {/* Header */}
        <div className="p-4 border-b border-slate-700">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-lg font-semibold text-slate-100 font-mono">{struct.name}</h3>
          </div>
        ...
        <div className="p-4">
          {activeTab === 'details' && (
            <div className="space-y-4">
              {struct.generic_type_params && struct.generic_type_params.length > 0 && (
                <div>
                  <h4 className="text-sm font-medium text-slate-300 mb-2">Generic Type Parameters</h4>
                  <div className="space-y-1">
                    {struct.generic_type_params.map((param: any, index: number) => (
                      <div key={index} className="text-sm text-slate-400 font-mono bg-slate-800/50 px-3 py-2 rounded">
                        <span className="text-slate-300">T{index}</span>
                        {param.constraints && param.constraints.length > 0 ? (
                          <span className="text-slate-500">: {param.constraints.join(', ')}</span>
              ...
              {struct.fields && struct.fields.length > 0 && (
                <div>
                  <h4 className="text-sm font-medium text-slate-300 mb-2">Fields</h4>
                  <div className="space-y-1">
              ...
              {struct.abilities && struct.abilities.length > 0 && (
                <div>
                  <h4 className="text-sm font-medium text-slate-300 mb-2">Abilities</h4>
                  <div className="flex flex-wrap gap-1">
                    {struct.abilities.map((ability: string, index: number) => (
                      <span key={index} className="px-2 py-1 rounded text-xs bg-slate-700 text-slate-300">
                        {ability}
                      </span>
                    ))}
                  </div>
  ...
  return (
    <div className="space-y-8">
      <header className="space-y-2">
        <h1 className="text-3xl font-semibold tracking-tight">{t('aptosAbi.title')}</h1>
      ...
      <section className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          >
            {moduleLoading ? t('aptos.module.loading') : t('aptosAbi.actions.loadAbi')}
          </button>
        </div>
      ...
      {moduleAbi && (
        <section className="space-y-8">
          <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-6">
            <h2 className="text-xl font-semibold text-slate-100 mb-4">{t('aptosAbi.moduleInfo.title')}</h2>
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <h3 className="text-sm font-medium text-slate-300 mb-2">{t('aptosAbi.moduleInfo.address')}</h3>
                <p className="text-sm text-slate-400 font-mono">{moduleAbi.address}</p>
              </div>
              <div>
                <h3 className="text-sm font-medium text-slate-300 mb-2">{t('aptosAbi.moduleInfo.name')}</h3>
                <p className="text-sm text-slate-400 font-mono">{moduleAbi.name}</p>
              </div>
            </div>
            
            <div className="mt-6 pt-4 border-t border-slate-700">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-medium text-slate-300">{t('aptosAbi.moduleInfo.overview')}</h3>
                <button
                  onClick={toggleAllSections}
                  className="px-3 py-1 rounded text-xs font-medium bg-slate-700 text-slate-300 hover:bg-slate-600 transition-colors"
                >
                  {Object.values(expandedSections).every(Boolean) ? t('aptosAbi.actions.collapseAll') : t('aptosAbi.actions.expandAll')}
                </button>
              </div>
              <div className="flex flex-wrap gap-3">
                <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/20 text-blue-300 text-sm">
                  <span className="w-2 h-2 rounded-full bg-blue-400"></span>
                  {entryFunctions.length} {t('aptosAbi.moduleInfo.entryFunctions')}
                </div>
                <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-green-500/20 text-green-300 text-sm">
                  <span className="w-2 h-2 rounded-full bg-green-400"></span>
                  {viewFunctions.length} {t('aptosAbi.moduleInfo.viewFunctions')}
                </div>
          ...
          >
            {entryFunctions.length > 0 ? (
              ...
              <div className="rounded-lg border border-slate-700 bg-slate-900/30 p-6 text-center">
                <p className="text-slate-400">{t('aptosAbi.entryFunctions.empty')}</p>
              </div>
            )}
          </AccordionSection>
          ...
          >
            {viewFunctions.length > 0 ? (
              ...
              <div className="rounded-lg border border-slate-700 bg-slate-900/30 p-6 text-center">
                <p className="text-slate-400">{t('aptosAbi.viewFunctions.empty')}</p>
              </div>
            )}
          </AccordionSection>
          ...
          {moduleBytecode.bytecode && (
            ...
            >
              <div className="rounded-lg border border-slate-700 bg-slate-950/70 p-4 relative">
                <button
                  onClick={() => copyToClipboard(moduleBytecode.bytecode, 'bytecode')}
                  className="absolute top-2 right-2 px-3 py-1 text-xs font-medium bg-slate-700 hover:bg-slate-600 text-slate-300 rounded transition-colors z-10"
                >
                  {copiedId === 'bytecode' ? t('aptosAbi.code.copied') : t('aptosAbi.code.copy')}
                </button>
                <div className="mb-3">
                  <h3 className="text-sm font-medium text-slate-300 mb-2">{t('aptosAbi.bytecode.hex')}</h3>
